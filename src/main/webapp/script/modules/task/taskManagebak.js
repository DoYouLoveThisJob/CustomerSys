			...
			.dataTable(
				{
					...
					"createdRow" : function(row, data, index) {
						...
						if(data.jobStatus==0){
							$('td', row).eq(7).html(startButton+"&nbsp;"+updateButton);
						}else if(data.jobStatus==1){
							$('td', row).eq(7).html(stopButton+"&nbsp;"+runNowButton);
						}
...
function  updateJob(taskId){
	window.location.href="/task/addTaskPage.action?taskId="+taskId;
}
function  showJob(taskId){
	window.location.href="/task/addTaskPage.action?taskId="+taskId+"&op=show";
}
